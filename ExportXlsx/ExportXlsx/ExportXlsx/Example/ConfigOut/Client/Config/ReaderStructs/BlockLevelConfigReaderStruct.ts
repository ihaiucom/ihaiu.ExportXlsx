...
namespace configs
{
	...
	export class BlockLevelConfigReaderStruct extends CsvConfigRender<    BlockLevelConfig      >
	{
        ...
        ParseCsv(csv: string[] )
        {
			...
			config.current_msg =  toStringArray(       csvGetString(csv,  this.GetHeadIndex(  "current_msg"  )   )   );
			config.next_msg =  toStringArray(       csvGetString(csv,  this.GetHeadIndex(  "next_msg"  )   )   );
